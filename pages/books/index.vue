<template>
  <Page :title="title">
    <template #action>
      <gv-button :href="$resolve.book()" primary>
        ...
        <gv-row>
          <gv-col>
            <gv-input
              ...
              :placeholder="$t('page.books.filter')"
            >
              <template #trailing>
      ...
      <gv-table>
        ...
        <template #body>
          <gv-tr v-for="item in filtered.list" :key="item._id">
            ...
            <gv-td>
              {{ item.releaseDate | timestamp($i18n.defaultLocale) }}
            </gv-td>
            <gv-td fit>
              <gv-button-group sm>
                <gv-button :href="$resolve.book(item._id)" info>
                  {{ $t("action.update") }}
                </gv-button>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { label: this.$t("label.release_date") },
        { fit: true },
      ],
    };
  },
  ...
  methods: {
    ...
    async openConfirmation(id) {
      this.restore.hide();
      await this.$service.book.find(id);
      this.confirmation.open();
    },
    async onConfirm() {
      await this.$service.book.delete();
      this.restore.show();
    },
    async onUndo() {
      await this.$service.book.restore();
    },
    async onHide() {
      await this.$service.book.clear();
    },
  },
};
</script>
